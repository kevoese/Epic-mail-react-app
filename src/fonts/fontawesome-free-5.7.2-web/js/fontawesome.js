  ...
  function findIcon(iconName, prefix) {
    return new picked(function(resolve, reject) {
      ...
      if (iconName && prefix && !config.showMissingIcons) {
        reject(
          new MissingIcon(
            'Icon is missing for prefix '.concat(prefix, ' with icon name ').concat(iconName)
          )
        );
      } else {
        resolve(val);
  ...
  function replaceForPosition(node, position) {
    ...
    return new picked(function(resolve, reject) {
      ...
      if (alreadyProcessedPseudoElement && !fontFamily) {
        ...
        var prefix = ~['Light', 'Regular', 'Solid', 'Brands'].indexOf(fontFamily[1])
          ? STYLE_TO_PREFIX[fontFamily[1].toLowerCase()]
          : FONT_WEIGHT_TO_PREFIX[fontWeight];
        var iconName = byUnicode(
          prefix,
          toHex(content.length === 3 ? content.substr(1, 1) : content)
        ...
        if (
          ...
          alreadyProcessedPseudoElement.getAttribute(DATA_ICON) !== iconName
        ) {
